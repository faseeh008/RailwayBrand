...
export async function buildMinimalistic(sessionData: BrandSessionData): Promise<string> {
	...
	let templatePath = join(process.cwd(), 'react-templates', 'Minimalistic', 'build', 'index.html');
	let html: string;
	let buildDir: string;
	
	try {
		html = readFileSync(templatePath, 'utf-8');
		buildDir = join(process.cwd(), 'react-templates', 'Minimalistic', 'build');
	} catch (e) {
		// Fallback to source index.html if build doesn't exist
		templatePath = join(process.cwd(), 'react-templates', 'Minimalistic', 'index.html');
		html = readFileSync(templatePath, 'utf-8');
		buildDir = join(process.cwd(), 'react-templates', 'Minimalistic');
	}
	...
	if (cssMatch) {
		...
		try {
			...
			console.warn('[buildMinimalistic] Failed to inline CSS:', e);
		}
	}
	...
	if (jsMatch) {
		...
		try {
			...
			console.warn('[buildMinimalistic] Failed to inline JS:', e);
		}
	}
