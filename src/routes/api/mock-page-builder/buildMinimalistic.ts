...
export async function buildMinimalistic(sessionData: BrandSessionData): Promise<string> {
	...
	let templateInfo;
	try {
		templateInfo = findTemplatePath('Minimalistic');
	} catch (error) {
		console.error('[buildMinimalistic] ❌ Failed to find template:', error);
		throw error;
	}
	...
	try {
		...
		console.log(`[buildMinimalistic] ✅ Successfully read template from ${templateInfo.isBuild ? 'build' : 'source'} directory`);
	} catch (e) {
		console.error('[buildMinimalistic] ❌ Failed to read template file:', e);
	...
	if (cssMatch) {
		...
		if (existsSync(fullCssPath)) {
			...
			if (!templateInfo.isBuild) {
				console.warn('[buildMinimalistic] ⚠️ Using source template - CSS may not be available');
			}
		}
	} else {
		if (!templateInfo.isBuild) {
			console.warn('[buildMinimalistic] ⚠️ No CSS link found in source template - page may appear unstyled');
		}
	}
	...
	if (jsMatch) {
		...
		if (sourceJsMatch && !templateInfo.isBuild) {
			console.error('[buildMinimalistic] ❌ Source template uses /src/main.tsx which cannot be loaded in blob URLs');
			console.error('[buildMinimalistic] ❌ This will result in a blank page. Build directories must be available.');
			console.error('[buildMinimalistic] ❌ The React app will not load because /src/main.tsx requires a dev server');
		}
	}
  ...
  <script>
	...
	if (!html || html.trim().length < 100) {
		console.error('[buildMinimalistic] ❌ Generated HTML is too short or empty');
		console.error('[buildMinimalistic] HTML length:', html?.length || 0);
		throw new Error('Generated HTML is empty or invalid. Template processing may have failed.');
	}
	
	// Log HTML info for debugging
	console.log(`[buildMinimalistic] Generated HTML length: ${html.length} characters`);
	console.log(`[buildMinimalistic] HTML contains __BRAND_CONFIG__: ${html.includes('__BRAND_CONFIG__')}`);
	console.log(`[buildMinimalistic] HTML contains root div: ${html.includes('<div id="root">')}`);
	
	if (!templateInfo.isBuild) {
		console.warn('[buildMinimalistic] ⚠️ Using source template - page may be blank in blob URLs');
		console.warn('[buildMinimalistic] ⚠️ Build directories should be available for production use');
	}
