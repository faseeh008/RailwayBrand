...
export class EnhancedComplianceAnalyzer {
  ...
  async analyzeCompliance(scrapedData, brandGuidelines) {
    ...
    if (hasColors) {
      // DEBUG: Add comprehensive data logging for colors
      console.log('🎨 DEBUG: SCRAPED COLORS DATA:', JSON.stringify(scrapedData.colors, null, 2));
      console.log('🎨 DEBUG: Brand colors:', brandGuidelines.colors);
    ...
    if (hasTypography) {
      // DEBUG: Add comprehensive data logging
      console.log('📝 DEBUG: SCRAPED TYPOGRAPHY DATA:', JSON.stringify(scrapedData.typography, null, 2));
      console.log('📝 DEBUG: Brand typography:', brandGuidelines.typography);
    ...
    // Calculate overall score with weighted categories for FAIRNESS
    const categoryWeights = {
      colors: 0.35,      // 35% - Colors are the most visible brand element
      typography: 0.30,  // 30% - Typography is important but less critical
      logo: 0.25,        // 25% - Logo is important but presence isn't everything  
      layout: 0.05,     // 5% - Layout is secondary
      imagery: 0.05      // 5% - Imagery is secondary
    };
    
    let totalWeight = 0;
    let weightedSum = 0;
    
    // Calculate weighted average (much more fair)
    for (const [category, score] of Object.entries(analysis.categoryScores)) {
      if (score > 0) {
        const weight = categoryWeights[category] || 0.1; // Default weight
        weightedSum += score * weight;
        totalWeight += weight;
      }
    }
    
    analysis.overallScore = totalWeight > 0 ? weightedSum / totalWeight : 0.7; // Default to 70% if no categories
    
    // Apply fairness bonuses for good partial compliance
    analysis.overallScore = this.applyFairnessBonuses(analysis.overallScore, analysis.categoryScores, scrapedData);
    
    // Collect available scores for logging
    const availableCategoryScores = Object.values(analysis.categoryScores).filter(score => score > 0);
    ...
    console.log('📊 Final analysis results:', {
      ...
      availableScores: availableCategoryScores,
  ...
  /**
   * Apply fairness bonuses to overall score
   * Rewards partial compliance and reduces harsh penalties
   */
  applyFairnessBonuses(baseScore, categoryScores, scrapedData) {
    let bonus = 0;
    
    console.log('⚖️ Applying fairness bonuses to score:', baseScore);
    
    // Bonus for having good color usage
    if (categoryScores.colors > 0.7) {
      bonus += 0.05;
      console.log('  ✅ Color bonus: +5%');
    }
    
    // Bonus for having reasonable typography
    if (categoryScores.typography > 0.6) {
      bonus += 0.05;
      console.log('  ✅ Typography bonus: +5%');
    }
    
    // Bonus for having substantial content
    if (scrapedData.elements && scrapedData.elements.length > 10) {
      bonus += 0.05;
      console.log('  ✅ Content richness bonus: +5%');
    }
    
    // Bonus for using brand colors (even if not perfect)
    if (scrapedData.colors && scrapedData.colors.palette && scrapedData.colors.palette.length > 0) {
      bonus += 0.03;
      console.log('  ✅ Has colors bonus: +3%');
    }
    
    // Bonus for having some typography detected (reduces false-negative penalty)
    if (scrapedData.typography && (scrapedData.typography.primary || scrapedData.typography.secondary)) {
      bonus += 0.02;
      console.log('  ✅ Typography detected bonus: +2%');
    }

    const finalScore = Math.min(1.0, baseScore + bonus);
    
    console.log(`🎁 Final fairness score: ${Math.round(baseScore * 100)}% + ${Math.round(bonus * 100)}% = ${Math.round(finalScore * 100)}%`);
