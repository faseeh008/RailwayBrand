...
export const getGoogleAIAPIKey = () => {
  // Server-side: use process.env - check multiple possible variable names
  if (typeof process !== 'undefined' && process.env) {
    // Check for Google_Gemini_Api first (as specified by user)
    if (process.env.Google_Gemini_Api) {
      console.log('🔑 Google AI API key found in process.env.Google_Gemini_Api');
    ...
    if (process.env.GOOGLE_AI_API_KEY) {
      console.log('🔑 Google AI API key found in process.env.GOOGLE_AI_API_KEY');
      return process.env.GOOGLE_AI_API_KEY;
    }
  }
  // Client-side: use import.meta.env (but this should be avoided for API keys)
  if (typeof import.meta !== 'undefined' && import.meta.env) {
    if (import.meta.env.VITE_GOOGLE_GEMINI_API) {
      console.log('🔑 Google AI API key found in import.meta.env.VITE_GOOGLE_GEMINI_API');
      return import.meta.env.VITE_GOOGLE_GEMINI_API;
    }
    if (import.meta.env.VITE_GOOGLE_AI_API_KEY) {
      console.log('🔑 Google AI API key found in import.meta.env.VITE_GOOGLE_AI_API_KEY');
