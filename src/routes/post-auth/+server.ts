	...
	if (session.user.id) {
		try {
			...
			if (u?.disabled) {
				...
				console.log(
					`Redirecting to: /auth/login?error=disabled&message=${encodeURIComponent('Account is disabled. Please contact an administrator.')}`
				);
				...
				throw redirect(
					303,
					`/auth/login?error=disabled&message=${encodeURIComponent('Account is disabled. Please contact an administrator.')}`
				);
			}
		} catch {}
	}
