<script lang="ts">
	...
	function downloadCreative(creative: (typeof mockCreatives)[0], format: string) {
		alert(
			`Downloading ${creative.title} as ${format.toUpperCase()}. This will be implemented in the full version!`
		);
	}

	function regenerateCreative(creative: (typeof mockCreatives)[0]) {
				...
				<CardContent class="space-y-4">
					<div class="space-y-2">
						...
						>
							...
							{#each campaignTypes as type (type)}
					...
					<div class="space-y-2">
						...
						>
							...
							{#each callsToAction as action (action)}
		...
		<div class="space-y-6">
			...
			{:else if showCreatives}
				<div class="space-y-6">
					...
					{#each mockCreatives as creative (creative.id)}
						<Card>
							<CardHeader>
