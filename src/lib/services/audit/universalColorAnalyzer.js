...
export class UniversalColorAnalyzer {
  ...
  normalizeColorStructure(colors) {
    ...
    // FIX: Handle simple array of colors (direct RGB values from scraper)
    if (Array.isArray(colors)) {
      console.log('🎨 Detected color array format, normalizing...');
      normalized.palette = this.normalizeColorArray(colors);
      normalized.palette.forEach(color => normalized.allColors.add(color));
      return normalized;
    }
  ...
  normalizeColorToHex(color) {
    ...
    // Handle RGB format (e.g., 'rgb(0, 0, 0)' or 'rgb(255, 255, 255)')
  ...
  rgbToHex(rgb) {
    try {
      ...
      console.warn('⚠️ Failed to convert RGB to hex:', rgb, error);
    }
    return null;
  }
  ...
  analyzeColorCategory(scraped, brand, category) {
    ...
    brandColors.forEach(brandColor => {
      ...
      if (bestMatch > 0.8) {
        brandColorsUsed++;
      } else if (bestMatch < 0.3 && brandColors.length > 0) {
        // Only create issue if we have actual brand colors defined for this category
        // Don't flag missing secondary if it doesn't exist or is same as primary
        const isActualSecondary = category === 'secondary' && 
          this.isSecondaryActuallyDifferent(normalizedBrand);
        
        // Don't create generic "missing" issues - be specific
        issues.push({
          category: 'colors',
          type: `${category}_color_missing`,
          severity: category === 'primary' ? 'medium' : 'low', // Reduced severity
          message: `${this.capitalize(category)} brand color could be used more prominently`,
          suggestion: `Consider incorporating ${brandColor} in ${category} UI elements`
        });
      }
    });
  ...
  analyzeColorPalette(scraped, brand) {
    ...
    // IMPROVED: Only flag if brand colors are actually being used but poorly distributed
    if (finalScore < 0.3 && brandColorsUsed === 0 && scraped.allColors.size > 0) {
      // Only flag if NO brand colors are being used at all
      issues.push({
        category: 'colors',
        type: 'brand_colors_not_detected',
        severity: 'high',
        message: 'No brand colors detected on the page',
        suggestion: 'Use the official brand color palette'
      });
    }
    // REMOVED: Don't flag vague "underutilized" issues
    
    return { score: finalScore, issues };
  }

  /**
   * Check if secondary color is actually different from primary
   */
  isSecondaryActuallyDifferent(normalizedBrand) {
    if (normalizedBrand.secondary.length === 0) return false;
    
    // Get primary color
    const primaryColor = normalizedBrand.primary && normalizedBrand.primary.length > 0 
      ? normalizedBrand.primary[0] 
      : null;
    
    // Get secondary color
    const secondaryColor = normalizedBrand.secondary[0];
    
    // Check if they're the same
    if (primaryColor && secondaryColor) {
      const similarity = this.colorSimilarity(primaryColor, secondaryColor);
      return similarity < 0.95; // Return true if they're different (>95% similarity means same)
    }
    
    return true; // If no primary, assume secondary is different
  }
