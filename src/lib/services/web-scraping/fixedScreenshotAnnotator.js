...
export class FixedScreenshotAnnotator {
  ...
  async createTargetedAnnotations(screenshotPath, auditResults, elementPositions) {
    ...
    const marginLeft = 300; // Space for legend on left
    const marginRight = 250; // Space for score on right
    const canvas = createCanvas(image.width + marginLeft + marginRight, image.height + marginTop + marginBottom);
    const ctx = canvas.getContext('2d');
    
    // White background + draw screenshot shifted down and right
    ctx.fillStyle = '#fff';
    ...
    // Add legend and score OUTSIDE the screenshot bounds to avoid overlap
    this.addTargetedLegend(ctx, -280, 20, groupedIssues, nonHighlightableIssues);
    this.addScoreOverlay(ctx, canvas.width + 20, 20, auditResults.overallScore);
  ...
  annotateCategoryIssues(ctx, category, issues, elementPositions, startCounter, marginTop = 0, marginLeft = 0, screenshotWidth = 1920, screenshotHeight = 1080) {
    ...
    issues.forEach(issue => {
      ...
      if (relevantElements.length > 0) {
        ...
        relevantElements.forEach(element => {
          this.highlightElement(ctx, element, issueConfig.color, issueCounter, marginTop, marginLeft, scaleX, scaleY);
        });
        
        // Add label for the first relevant element
        if (relevantElements.length > 0) {
          this.addIssueLabel(ctx, relevantElements[0], issue, issueCounter, issueConfig.color, marginTop, marginLeft, scaleX, scaleY);
        }
  ...
  highlightElement(ctx, element, color, issueNumber, marginTop = 0, marginLeft = 0, scaleX = 1, scaleY = 1) {
    ...
    // Adjust position for margin offsets
    const adjustedX = scaledX + marginLeft;
    ...
    ctx.lineWidth = 3;
    ctx.setLineDash([6, 3]);
    ctx.strokeRect(adjustedX - 1, adjustedY - 1, scaledWidth + 2, scaledHeight + 2);
    ctx.setLineDash([]);
    
    // Draw issue number badge with collision avoidance
    this.drawIssueBadge(ctx, adjustedX, adjustedY, scaledWidth, scaledHeight, issueNumber, color);
  }
  ...
  addIssueLabel(ctx, element, issue, issueNumber, color, marginTop = 0, marginLeft = 0, scaleX = 1, scaleY = 1) {
