...
export class EnhancedWebScraper {
  ...
  async extractComprehensiveData(page, url) {
    return await page.evaluate((currentUrl) => {
      ...
      // ENHANCED: Extract fonts from computed styles AND document styling with multiple fallbacks
      ...
      // Method 1: Get fonts from all visible elements
      elements.forEach(el => {
        if (el.styles && el.styles.fontFamily) {
          el.styles.fontFamily.split(',').forEach(font => {
            ...
            if (cleanFont && cleanFont !== 'serif' && cleanFont !== 'sans-serif' && cleanFont !== 'monospace') {
              ...
              // Track font usage
              fontData.usage[cleanFont] = (fontData.usage[cleanFont] || 0) + 1;
            }
          });
          
          // Track font weights
          if (el.styles.fontWeight) {
            fontData.weights.add(el.styles.fontWeight);
          }
          if (el.styles.fontSize) {
            fontData.fontSize.push(el.styles.fontSize);
          }
        }
      });
      
      // Method 2: Check body and root computed styles (fallback for empty pages)
      if (fontSet.size === 0) {
        try {
          ...
          if (rootStyle.fontFamily && fontSet.size === 0) {
            rootStyle.fontFamily.split(',').forEach(font => {
              const cleanFont = font.trim().replace(/['"]/g, '');
              if (cleanFont && cleanFont !== 'serif' && cleanFont !== 'sans-serif' && cleanFont !== 'monospace') {
                fontSet.add(cleanFont);
                fontData.primary = cleanFont;
          ...
          console.warn('⚠️ Failed to extract fonts from computed styles:', e);
        }
      }
      
      // Method 3: Parse stylesheets for @font-face rules
      if (fontSet.size === 0) {
        try {
          ...
          console.warn('⚠️ Could not parse CSS stylesheets:', error);
        }
      }
      
      // Log font detection results
      console.log(`🔤 Font detection: ${fontSet.size} fonts found`, {
        primary: fontData.primary,
        secondary: fontData.secondary,
        allFonts: Array.from(fontSet)
      });
