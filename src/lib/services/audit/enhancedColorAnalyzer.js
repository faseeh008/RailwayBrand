...
export class EnhancedColorAnalyzer {
  ...
  normalizeScrapedColors(scrapedColors) {
    console.log('🎨 DEBUG: normalizeScrapedColors input:', scrapedColors);
    
    // FIX: Handle simple array of colors (direct RGB values from scraper)
    if (Array.isArray(scrapedColors)) {
      console.log('🎨 Detected color array format (enhancedColorAnalyzer), normalizing...');
      const normalized = {
        primary: null,
        secondary: [],
        palette: this.normalizeColorArray(scrapedColors),
        allColors: this.normalizeColorArray(scrapedColors)
      };
      console.log('🎨 Normalized array to:', normalized);
      return normalized;
    }
