<script lang="ts">
	...
	const templateSets = [
		{
			...
			description: 'Arial font, gradient backgrounds'
		},
		{
			value: 'modern-minimal',
			label: 'Modern Minimal',
			description: 'Helvetica Neue, clean whitespace'
		},
		{
			value: 'corporate-professional',
			label: 'Corporate Professional',
			description: 'Navy blue, structured layout'
		},
		{
			value: 'creative-bold',
			label: 'Creative Bold',
			description: 'Vibrant colors, dynamic layouts'
		}
	...
	onMount(async () => {
		try {
			...
			const savedFlag = sessionStorage.getItem('preview_brand_saved');
			if (savedFlag === 'true') {
			...
			if (!guidelineId) {
				...
				try {
					const response = await fetch(
						`/api/brand-guidelines/by-name?brandName=${encodeURIComponent(brandData.brandName)}`
					);
					if (response.ok) {
				...
				try {
					...
					if (response.ok) {
						...
						if (result.success && result.guideline) {
							...
							if (
								guideline.logoFiles &&
								(!brandData.logoFiles || brandData.logoFiles.length === 0)
							) {
								try {
									const logoFiles =
										typeof guideline.logoFiles === 'string'
							...
							if (guideline.shortDescription)
								brandData.short_description = guideline.shortDescription;
							...
							if (guideline.contactInfo) {
								try {
									const contactInfo =
										typeof guideline.contactInfo === 'string'
							...
							if (guideline.structuredData) {
								try {
									...
									if (structuredData.selectedMood && !brandData.selectedMood)
										brandData.selectedMood = structuredData.selectedMood;
									if (structuredData.selectedAudience && !brandData.selectedAudience)
										brandData.selectedAudience = structuredData.selectedAudience;
									if (structuredData.brandValues && !brandData.brandValues)
										brandData.brandValues = structuredData.brandValues;
									if (structuredData.customPrompt && !brandData.customPrompt)
										brandData.customPrompt = structuredData.customPrompt;
									if (structuredData.contact && !brandData.contact)
										brandData.contact = structuredData.contact;
								} catch (parseError) {
			...
			const existingSlidesStep = brandData.stepHistory?.find(
	...
	async function toggleEdit() {
		if (isEditable) {
			...
			if (iframeRef && slides[currentSlide]) {
				try {
					const currentHtml =
						iframeRef.contentDocument?.documentElement?.outerHTML || slides[currentSlide].html;
	...
	function cleanupEditingMode(iframeRef: HTMLIFrameElement | undefined) {
		...
		try {
			...
			try {
				const handles = iframeDoc.querySelectorAll(
					'.selection-handles, .selection-handle, .resize-handle'
				);
				handles.forEach((handle: any) => {
					try {
						if (handle.updateInterval) {
	...
	async function revertChanges() {
		...
		const confirmed = confirm(
			'Are you sure you want to discard all your edits? This cannot be undone.'
		);
		if (!confirmed) return;
	...
	function enableTextEditing(iframeDoc: Document) {
		...
		const editableDivs = Array.from(allDivs).filter((div: any) => {
			...
			return (
				!div.classList.contains('color-swatch') &&
			...
			); // Only include divs with text content
		});
    ...
    `;
		...
		insertGroup.appendChild(
			createProfessionalButton(iframeDoc, 'text', 'Text', () =>
				addTextElement(iframeDoc, iframeInteract)
			)
		);
		insertGroup.appendChild(
			createProfessionalButton(iframeDoc, 'square', 'Rectangle', () =>
				addShapeElement(iframeDoc, iframeInteract, 'rectangle')
			)
		);
		insertGroup.appendChild(
			createProfessionalButton(iframeDoc, 'circle', 'Circle', () =>
				addShapeElement(iframeDoc, iframeInteract, 'circle')
			)
		);
		insertGroup.appendChild(
			createProfessionalButton(iframeDoc, 'line', 'Line', () =>
				addLineElement(iframeDoc, iframeInteract)
			)
		);
		insertGroup.appendChild(
			createProfessionalButton(iframeDoc, 'image', 'Image', () =>
				addImageElement(iframeDoc, iframeInteract)
			)
		);
		...
		historyGroup.appendChild(
			createProfessionalButton(iframeDoc, 'undo', 'Undo', () =>
				undoAction(iframeDoc, iframeInteract)
			)
		);
		historyGroup.appendChild(
			createProfessionalButton(iframeDoc, 'redo', 'Redo', () =>
				redoAction(iframeDoc, iframeInteract)
			)
		);
		...
		alignHGroup.appendChild(
			createProfessionalButton(iframeDoc, 'alignLeft', 'Align Left', () =>
				alignSelectedElements(iframeDoc, 'left')
			)
		);
		alignHGroup.appendChild(
			createProfessionalButton(iframeDoc, 'alignCenter', 'Align Center', () =>
				alignSelectedElements(iframeDoc, 'center')
			)
		);
		alignHGroup.appendChild(
			createProfessionalButton(iframeDoc, 'alignRight', 'Align Right', () =>
				alignSelectedElements(iframeDoc, 'right')
			)
		);
		...
		alignVGroup.appendChild(
			createProfessionalButton(iframeDoc, 'alignTop', 'Align Top', () =>
				alignSelectedElements(iframeDoc, 'top')
			)
		);
		alignVGroup.appendChild(
			createProfessionalButton(iframeDoc, 'alignMiddle', 'Align Middle', () =>
				alignSelectedElements(iframeDoc, 'middle')
			)
		);
		alignVGroup.appendChild(
			createProfessionalButton(iframeDoc, 'alignBottom', 'Align Bottom', () =>
				alignSelectedElements(iframeDoc, 'bottom')
			)
		);
		...
		// Snap to Grid toggle
		const snapToggle = createProfessionalButton(
			iframeDoc,
			'grid',
			snapToGridEnabled ? 'Snap: On' : 'Snap: Off',
			() => {
	...
	function createProfessionalButton(
		iframeDoc: Document,
		iconName: string,
		tooltip: string,
		onClick: () => void
	...
	function enableLayoutEditing(iframeDoc: Document) {
		...
		const doubleClickHandler = (e: MouseEvent) => {
			...
			if (
				elementToEdit.tagName.match(/^(H[1-6]|P|SPAN|DIV|LI|TD|TH|A)$/i) &&
				...
				elementToEdit.addEventListener(
					'blur',
					() => {
						saveToHistory(iframeDoc);
						updateSlideContent();
		...
		console.log(
			'✅ Editing mode enabled - click to select, double-click to edit text, drag to move!'
		);
	}
	...
	function initializeInteractForElement(
		element: HTMLElement,
		iframeDoc: Document,
		iframeInteract: any
	) {
		...
		interactInstance.draggable({
			...
			ignoreFrom:
				'.selection-handles, .resize-handle, .toolbar-btn, button, input, textarea, select', // Don't drag from UI elements
		...
		interactInstance.resizable({
			...
			listeners: {
				...
				move(event: any) {
					const target = event.target;
					let x =
						parseFloat(target.dataset.x || target.style.left || '0') + (event.deltaRect?.left || 0);
					let y =
						parseFloat(target.dataset.y || target.style.top || '0') + (event.deltaRect?.top || 0);
	...
	function selectElement(element: HTMLElement, iframeDoc: Document) {
		...
		if (
			element.classList.contains('selection-handles') ||
	...
	function alignSelectedElements(
		...
		const selected = Array.from(
			iframeDoc.querySelectorAll('.editable-element.selected')
		) as HTMLElement[];
	...
	async function syncToDatabase(updatedBrandData: any, force = false) {
		if (!force && !hasSavedGuideline) {
			console.log('⏭️ Skipping database sync - guideline not saved yet');
			return;
		}
	...
	async function downloadPPTX() {
		try {
			...
			const name =
				(brandData.brand_name || brandData.brandName || 'Brand') + '-Brand-Guidelines.pptx';
			a.href = url;
			a.download = name;
	...
	async function downloadPDF() {
		try {
			...
			const a = document.createElement('a');
			const name =
				(brandData.brand_name || brandData.brandName || 'Brand') + '-Brand-Guidelines.pdf';
			a.href = url;
			a.download = name;
			document.body.appendChild(a);
			a.click();
