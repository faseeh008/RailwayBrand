...
export function getTypographyStyle(
  ...
  if (!typography || !typography.fontHierarchy) {
    ...
    return {
      fontFamily: typography?.primaryFont || 'Arial, sans-serif',
  ...
  if (hierarchy) {
    return {
      fontFamily: hierarchy.font || typography.primaryFont || 'Arial, sans-serif',
...
/**
 * Get CSS variable names for typography
 */
export function getTypographyCSSVars(label: string): {
  fontFamily: string;
  fontSize: string;
  fontWeight: string;
} {
  ...
  return {
    fontFamily: `var(--font-${normalizedLabel}-family, var(--font-primary, Arial, sans-serif))`,
    fontSize: `var(--font-${normalizedLabel}-size, 16px)`,
    fontWeight: `var(--font-${normalizedLabel}-weight, 400)`,
  };
}
