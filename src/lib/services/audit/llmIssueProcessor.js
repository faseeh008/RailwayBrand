...
`;
  ...
  /**
   * Filter out vague or incorrect issues
   */
  filterVagueIssues(issues) {
    return issues.filter(issue => {
      if (!issue) return false;
      
      const message = issue.message || issue.suggestion || '';
      const messageLower = message.toLowerCase();
      
      // Remove vague issues about "underutilized palette"
      if (messageLower.includes('underutilized') || 
          messageLower.includes('monotone') ||
          messageLower.includes('off-brand aesthetic') ||
          messageLower.includes('palette is') ||
          messageLower.includes('not being fully utilized')) {
        console.log(`🗑️ Removing vague issue: ${message.substring(0, 60)}...`);
        return false;
      }
      
      // Fix incorrect secondary color recommendations
      if (messageLower.includes('secondary color') && 
          (messageLower.includes('#000000') || messageLower.includes('#000 '))) {
        console.log(`🔄 Fixing incorrect secondary color issue: ${message}`);
        issue.message = 'Ensure consistent use of brand black color across all elements';
        issue.suggestion = 'Use the primary black color consistently for text, backgrounds, and key elements';
      }
      
      // Remove duplicate or very similar issues
      if (issue.message && issue.message.length < 20) {
        console.log(`🗑️ Removing too-short issue: ${issue.message}`);
        return false;
      }
  ...
  async processIssues(issues) {
    ...
    // IMPROVED: Filter out vague or incorrect issues
    const filteredIssues = this.filterVagueIssues(issues);
    console.log(`🗑️ Filtered ${issues.length} issues to ${filteredIssues.length} actionable issues`);
    
    try {
      // Group similar issues
      const groupedIssues = await this.groupSimilarIssues(filteredIssues);
      console.log(`📊 Grouped ${filteredIssues.length} issues into ${groupedIssues.length} groups`);
