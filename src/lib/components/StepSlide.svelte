...
function getIconAccent(index: number): string {
	...
	// Extract hierarchy from JSON format (enhanced generator)
	function extractHierarchyFromJSON(data: any): any {
		const hierarchy: any = {};
		
		if (data.font_hierarchy && Array.isArray(data.font_hierarchy)) {
			data.font_hierarchy.forEach((item: any) => {
				const label = (item.label || '').toLowerCase();
				const font = item.font || data.primary_font || '';
				const sizeStr = item.size || '';
				const size = parseInt(sizeStr.replace('px', '')) || (label === 'h1' ? 48 : label === 'h2' ? 32 : label === 'body' ? 16 : 16);
				const weightStr = item.weight || '';
				const weight = convertWeightToNumber(weightStr) || (label === 'h1' ? 700 : label === 'h2' ? 600 : 400);
				
				if (label === 'h1' || label === 'heading 1') {
					hierarchy.h1 = {
						fontName: font,
						size: size,
						weight: weight,
						exampleText: 'Heading 1 Example Text'
					};
				} else if (label === 'h2' || label === 'heading 2') {
					hierarchy.h2 = {
						fontName: font,
						size: size,
						weight: weight,
						exampleText: 'Heading 2 Example Text'
					};
				} else if (label === 'body' || label === 'body text') {
					hierarchy.body = {
						fontName: font || data.secondary_font || '',
						size: size,
						weight: weight,
						lineHeight: 1.6,
						exampleText: 'This is body text example showing how normal text appears with the selected typeface, size, and weight.'
					};
				}
			});
		}
	...
	// Extract font info from JSON format
	function extractFontInfoFromJSON(data: any): any {
		return {
			primary: {
				name: data.primary_font || ''
			},
			supporting: {
				name: data.secondary_font || ''
			}
		};
	}
	
	// Convert weight string to number
	function convertWeightToNumber(weight: string): number {
		if (!weight) return 400;
		const weightLower = weight.toLowerCase();
		if (weightLower.includes('bold') || weightLower.includes('700')) return 700;
		if (weightLower.includes('semibold') || weightLower.includes('600')) return 600;
		if (weightLower.includes('medium') || weightLower.includes('500')) return 500;
		if (weightLower.includes('regular') || weightLower.includes('400')) return 400;
		if (weightLower.includes('light') || weightLower.includes('300')) return 300;
		const numMatch = weight.match(/\d+/);
		return numMatch ? parseInt(numMatch[0]) : 400;
	}
	...
	<CardContent>
		{#if stepData}
			<div class="slide-content">
				...
				{:else if stepId === 'typography'}
					<div class="typography-slide">
						{#if typeof stepData === 'string'}
							...
							{@const hierarchy = parsedData?.font_hierarchy ? extractHierarchyFromJSON(parsedData) : extractTypographyHierarchy(stepData)}
							{@const fontInfo = parsedData ? extractFontInfoFromJSON(parsedData) : extractFontInfo(stepData)}
							...
							<!-- Prominent Typography Hierarchy Display: H1, H2, Body for Primary Font -->
							{#if primaryFontName && (hierarchy.h1 || hierarchy.h2 || hierarchy.body)}
								<div class="typography-hierarchy-prominent">
									<h3 class="section-title">Primary Font Hierarchy</h3>
									<div class="hierarchy-display-prominent">
										{#if hierarchy.h1}
											{@const h1Font = hierarchy.h1.fontName || primaryFontName || 'sans-serif'}
											{@const h1Family = getFontFamily(h1Font)}
											{@const h1Size = hierarchy.h1.size || 48}
											{@const h1Weight = hierarchy.h1.weight || 700}
											<div class="hierarchy-item-prominent h1-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 1</span>
													<span class="hierarchy-specs-prominent">{h1Font} • {h1Size}px / {h1Weight}</span>
												</div>
												<div 
													class="hierarchy-text-prominent"
													style="font-family: {h1Family}; font-size: {h1Size}px; font-weight: {h1Weight};"
												>
													{hierarchy.h1.exampleText || 'Heading 1 Example Text'}
												</div>
											</div>
										{/if}
										...
										{#if hierarchy.h2}
											...
											{@const h2Size = hierarchy.h2.size || 32}
											{@const h2Weight = hierarchy.h2.weight || 600}
											<div class="hierarchy-item-prominent h2-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 2</span>
													<span class="hierarchy-specs-prominent">{h2Font} • {h2Size}px / {h2Weight}</span>
												</div>
												<div 
													class="hierarchy-text-prominent"
													style="font-family: {h2Family}; font-size: {h2Size}px; font-weight: {h2Weight};"
												>
													{hierarchy.h2.exampleText || 'Heading 2 Example Text'}
												</div>
											</div>
										{/if}
										
										{#if hierarchy.body}
											{@const bodyFont = primaryFontName}
											{@const bodyFamily = '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'}
											{@const bodySize = hierarchy.body.size || 16}
											{@const bodyWeight = 400}
											{@const bodyLineHeight = hierarchy.body.lineHeight || 1.6}
											<div class="hierarchy-item-prominent body-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Body Text</span>
													<span class="hierarchy-specs-prominent">{bodyFont} • {bodySize}px / {bodyWeight}</span>
												</div>
												<div 
													class="hierarchy-text-prominent body-text-prominent"
													style="font-family: {bodyFamily}; font-size: {bodySize}px; font-weight: {bodyWeight}; line-height: {bodyLineHeight};"
												>
													{hierarchy.body.exampleText || 'This is body text example showing how normal text appears with the selected typeface, size, and weight. It demonstrates readability and visual hierarchy.'}
												</div>
											</div>
										{/if}
									</div>
							...
							<!-- Prominent Typography Hierarchy Display: H1, H2, Body for Secondary Font -->
							{#if supportingFontName && (hierarchy.h1 || hierarchy.h2 || hierarchy.body)}
								<div class="typography-hierarchy-prominent">
									<h3 class="section-title">Secondary Font Hierarchy</h3>
									<div class="hierarchy-display-prominent">
										{#if hierarchy.h1}
											{@const h1FontSecondary = supportingFontName}
											{@const h1FamilySecondary = supportingFontFamily}
											{@const h1SizeSecondary = hierarchy.h1.size || 48}
											{@const h1WeightSecondary = hierarchy.h1.weight || 700}
											<div class="hierarchy-item-prominent h1-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 1</span>
													<span class="hierarchy-specs-prominent">{h1FontSecondary} • {h1SizeSecondary}px / {h1WeightSecondary}</span>
												</div>
												<div 
													class="hierarchy-text-prominent"
													style="font-family: {h1FamilySecondary}; font-size: {h1SizeSecondary}px; font-weight: {h1WeightSecondary};"
												>
													{hierarchy.h1.exampleText || 'Heading 1 Example Text'}
												</div>
											</div>
										...
										{#if hierarchy.h2}
											{@const h2FontSecondary = supportingFontName}
											{@const h2FamilySecondary = supportingFontFamily}
											{@const h2SizeSecondary = hierarchy.h2.size || 32}
											{@const h2WeightSecondary = hierarchy.h2.weight || 600}
											<div class="hierarchy-item-prominent h2-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 2</span>
													<span class="hierarchy-specs-prominent">{h2FontSecondary} • {h2SizeSecondary}px / {h2WeightSecondary}</span>
												</div>
												<div 
													class="hierarchy-text-prominent"
													style="font-family: {h2FamilySecondary}; font-size: {h2SizeSecondary}px; font-weight: {h2WeightSecondary};"
												>
													{hierarchy.h2.exampleText || 'Heading 2 Example Text'}
												</div>
											</div>
										...
										{#if hierarchy.body}
											{@const bodyFontSecondary = supportingFontName}
											{@const bodyFamilySecondary = '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'}
											{@const bodySizeSecondary = hierarchy.body.size || 16}
											{@const bodyWeightSecondary = 400}
											{@const bodyLineHeightSecondary = hierarchy.body.lineHeight || 1.6}
											<div class="hierarchy-item-prominent body-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Body Text</span>
													<span class="hierarchy-specs-prominent">{bodyFontSecondary} • {bodySizeSecondary}px / {bodyWeightSecondary}</span>
												</div>
												<div 
													class="hierarchy-text-prominent body-text-prominent"
													style="font-family: {bodyFamilySecondary}; font-size: {bodySizeSecondary}px; font-weight: {bodyWeightSecondary}; line-height: {bodyLineHeightSecondary};"
												>
													{hierarchy.body.exampleText || 'This is body text example showing how normal text appears with the selected typeface, size, and weight. It demonstrates readability and visual hierarchy.'}
												</div>
											</div>
							...
							<!-- Only show hierarchy if available, otherwise show fallback -->
							{#if !hierarchy.h1 && !hierarchy.h2 && !hierarchy.body}
								...
								{/if}
					{:else}
						<!-- Fallback: Try to extract hierarchy from object format -->
						{@const hierarchyFromObj = extractHierarchyFromJSON(stepData)}
						{@const fontInfoFromObj = extractFontInfoFromJSON(stepData)}
						{@const primaryFontNameObj = fontInfoFromObj.primary?.name || stepData.primary_font || ''}
						{@const supportingFontNameObj = fontInfoFromObj.supporting?.name || stepData.secondary_font || ''}
						
						{#if hierarchyFromObj.h1 || hierarchyFromObj.h2 || hierarchyFromObj.body}
							<!-- Load fonts -->
							{#if hierarchyFromObj.h1?.fontName} {loadGoogleFont(hierarchyFromObj.h1.fontName)} {/if}
							{#if hierarchyFromObj.h2?.fontName} {loadGoogleFont(hierarchyFromObj.h2.fontName)} {/if}
							{#if hierarchyFromObj.body?.fontName} {loadGoogleFont(hierarchyFromObj.body.fontName)} {/if}
							{#if primaryFontNameObj} {loadGoogleFont(primaryFontNameObj)} {/if}
							{#if supportingFontNameObj} {loadGoogleFont(supportingFontNameObj)} {/if}
							
							{@const primaryFontFamilyObj = primaryFontNameObj ? getFontFamily(primaryFontNameObj) : 'sans-serif'}
							{@const supportingFontFamilyObj = supportingFontNameObj ? getFontFamily(supportingFontNameObj) : 'sans-serif'}
							
							<!-- Prominent Typography Hierarchy Display: H1, H2, Body for Primary Font -->
							{#if primaryFontNameObj && (hierarchyFromObj.h1 || hierarchyFromObj.h2 || hierarchyFromObj.body)}
								<div class="typography-hierarchy-prominent">
									<h3 class="section-title">Primary Font Hierarchy</h3>
									<div class="hierarchy-display-prominent">
										{#if hierarchyFromObj.h1}
											{@const h1Font = primaryFontNameObj}
											{@const h1Family = primaryFontFamilyObj}
											{@const h1Size = hierarchyFromObj.h1.size || 48}
											{@const h1Weight = hierarchyFromObj.h1.weight || 700}
											<div class="hierarchy-item-prominent h1-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 1</span>
													<span class="hierarchy-specs-prominent">{h1Font} • {h1Size}px / {h1Weight}</span>
												...
												<div 
													...
													style="font-family: {h1Family}; font-size: {h1Size}px; font-weight: {h1Weight};"
												>
													{hierarchyFromObj.h1.exampleText || 'Heading 1 Example Text'}
												</div>
											</div>
										{/if}
										
										{#if hierarchyFromObj.h2}
											{@const h2Font = primaryFontNameObj}
											{@const h2Family = primaryFontFamilyObj}
											{@const h2Size = hierarchyFromObj.h2.size || 32}
											{@const h2Weight = hierarchyFromObj.h2.weight || 600}
											<div class="hierarchy-item-prominent h2-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 2</span>
													<span class="hierarchy-specs-prominent">{h2Font} • {h2Size}px / {h2Weight}</span>
												...
												<div 
													...
													style="font-family: {h2Family}; font-size: {h2Size}px; font-weight: {h2Weight};"
												>
													{hierarchyFromObj.h2.exampleText || 'Heading 2 Example Text'}
												</div>
											</div>
										{/if}
										
										{#if hierarchyFromObj.body}
											{@const bodyFont = primaryFontNameObj}
											{@const bodyFamily = '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'}
											{@const bodySize = hierarchyFromObj.body.size || 16}
											{@const bodyWeight = 400}
											{@const bodyLineHeight = hierarchyFromObj.body.lineHeight || 1.6}
											<div class="hierarchy-item-prominent body-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Body Text</span>
													<span class="hierarchy-specs-prominent">{bodyFont} • {bodySize}px / {bodyWeight}</span>
												...
												<div 
													...
													style="font-family: {bodyFamily}; font-size: {bodySize}px; font-weight: {bodyWeight}; line-height: {bodyLineHeight};"
												>
													{hierarchyFromObj.body.exampleText || 'This is body text example showing how normal text appears with the selected typeface, size, and weight. It demonstrates readability and visual hierarchy.'}
												</div>
											</div>
										{/if}
									</div>
							...
							<!-- Prominent Typography Hierarchy Display: H1, H2, Body for Secondary Font -->
							{#if supportingFontNameObj && (hierarchyFromObj.h1 || hierarchyFromObj.h2 || hierarchyFromObj.body)}
								<div class="typography-hierarchy-prominent">
									<h3 class="section-title">Secondary Font Hierarchy</h3>
									<div class="hierarchy-display-prominent">
										{#if hierarchyFromObj.h1}
											{@const h1FontSecondary = supportingFontNameObj}
											{@const h1FamilySecondary = supportingFontFamilyObj}
											{@const h1SizeSecondary = hierarchyFromObj.h1.size || 48}
											{@const h1WeightSecondary = hierarchyFromObj.h1.weight || 700}
											<div class="hierarchy-item-prominent h1-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 1</span>
													<span class="hierarchy-specs-prominent">{h1FontSecondary} • {h1SizeSecondary}px / {h1WeightSecondary}</span>
										...
										{#if hierarchyFromObj.h2}
											{@const h2FontSecondary = supportingFontNameObj}
											{@const h2FamilySecondary = supportingFontFamilyObj}
											{@const h2SizeSecondary = hierarchyFromObj.h2.size || 32}
											{@const h2WeightSecondary = hierarchyFromObj.h2.weight || 600}
											<div class="hierarchy-item-prominent h2-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Heading 2</span>
													<span class="hierarchy-specs-prominent">{h2FontSecondary} • {h2SizeSecondary}px / {h2WeightSecondary}</span>
										...
										{#if hierarchyFromObj.body}
											{@const bodyFontSecondary = supportingFontNameObj}
											{@const bodyFamilySecondary = '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'}
											{@const bodySizeSecondary = hierarchyFromObj.body.size || 16}
											{@const bodyWeightSecondary = 400}
											{@const bodyLineHeightSecondary = hierarchyFromObj.body.lineHeight || 1.6}
											<div class="hierarchy-item-prominent body-prominent">
												<div class="hierarchy-meta-prominent">
													<span class="hierarchy-label-prominent">Body Text</span>
													<span class="hierarchy-specs-prominent">{bodyFontSecondary} • {bodySizeSecondary}px / {bodyWeightSecondary}</span>
				...
				{:else if typeof stepData === 'object' && stepId === 'typography'}
					<!-- Handle JSON format typography data -->
					{#if typeof stepData === 'object' && stepId === 'typography'}
						{@const hierarchy = extractHierarchyFromJSON(stepData)}
						{@const fontInfo = extractFontInfoFromJSON(stepData)}
						{@const primaryFontName = fontInfo.primary?.name || hierarchy.h1?.fontName || hierarchy.h2?.fontName}
						{@const supportingFontName = fontInfo.supporting?.name || hierarchy.body?.fontName}
						{@const primaryFontFamily = primaryFontName ? getFontFamily(primaryFontName) : 'sans-serif'}
						{@const supportingFontFamily = supportingFontName ? getFontFamily(supportingFontName) : 'sans-serif'}
						...
						<div class="typography-slide">
							...
							{#if primaryFontName && (hierarchy.h1 || hierarchy.h2 || hierarchy.body)}
								<div class="typography-hierarchy-prominent">
									...
									<div class="hierarchy-display-prominent">
										{#if hierarchy.h1}
											{@const h1Font = primaryFontName}
											{@const h1Family = primaryFontFamily}
											{@const h1Size = hierarchy.h1.size || 48}
											{@const h1Weight = hierarchy.h1.weight || 700}
											<div class="hierarchy-item-prominent h1-prominent">
												<div class="hierarchy-meta-prominent">
										...
										{#if hierarchy.h2}
											{@const h2Font = primaryFontName}
											{@const h2Family = primaryFontFamily}
											{@const h2Size = hierarchy.h2.size || 32}
											{@const h2Weight = hierarchy.h2.weight || 600}
											<div class="hierarchy-item-prominent h2-prominent">
												<div class="hierarchy-meta-prominent">
...
<style>
	...
	/* Prominent Typography Hierarchy Display */
	.typography-hierarchy-prominent {
		margin-bottom: 2rem;
		padding: 1.5rem;
		background: oklch(var(--card));
		border: 1px solid oklch(var(--border));
	...
	.hierarchy-text-prominent {
		color: oklch(var(--foreground));
		line-height: 1.2;
		word-wrap: break-word;
	}

	.hierarchy-text-prominent.body-text-prominent {
		line-height: 1.6;
		max-width: 100%;
		white-space: normal;
	}
	...
	.h2-prominent {
		border-left-color: oklch(var(--accent) / 0.7);
	}

	.body-prominent {
		border-left-color: oklch(var(--accent) / 0.5);
	}
